	...
	if($items.length > 0){
		$items.each(function(){
			...
			if($size > 0){
				...
				if(!$(this).find('img').hasClass('sv_img'))$(this).find('img').wrap('<div class="sv_img"></div>');
		...
		if($imgViewType == 'gallery' && $('.sv_check_item_wrap').length == 1){
			$('.btn_view_util.gal').addClass('on').siblings().removeClass('on');
			$('.sv_check_item_wrap').addClass('gallery');
		}
	}
